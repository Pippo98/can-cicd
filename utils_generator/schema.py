        ...
        for topic_name, _ in network.get_topics().items():
            for message_name, message_content in network.get_messages_by_topic(topic_name).items():
                ...
                if not message_name in messages_names:
                    ...
                    self.messages.append(Message(network.name, self.types, message_name, message_content["contents"], message_content.get("frequency", -1)))

        for message in self.messages:
            message.fields = sorted(message.fields, key=lambda d: d.bit_size, reverse=True)
            self.messages_size[message.name] = {0: [], 1: [], 2: [], 3: [], 4: [], 5: [], 6: [], 7: []}
            indice = 0
            inizio = 8
...
class Message:
    def __init__(self, filename, custom_types: dict, message_name: str, message_content: dict, frequency):
        global types_size
        types_size = {
            'bool': 1,
            'int8': 8,
...
class Field:
    def __init__(self, field_name: str, field_type, types: dict):
        self.name= field_name if not ':' in field_name else field_name.split(':')[1].strip()
        self._custom_type = field_type if not ':' in field_name else field_name.split(':')[0].strip()
...
class BitSet:
    def __init__(self,field_content):
        ...
        if 'items' in field_content:
            self.content = [Item(field, index) for index,field in enumerate(field_content['items'])]
